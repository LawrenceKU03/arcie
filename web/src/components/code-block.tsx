...
export function CodeBlock({ code, language, onCopy }: CodeBlockProps) {
  ...
  return (
    <figure className="response-code-block code-surface my-3">
      <figcaption className="flex items-center justify-between px-3.5 pt-2.5">
        <div className="flex min-w-0 items-center gap-2.5">
          <span
            aria-hidden="true"
            className="code-block-glyph"
          >
            {"</>"}
          </span>
          <span className="code-block-label truncate">
        ...
        <button
          ...
          onClick={handleCopy}
          className={cn(
            "code-copy-button flex h-7 w-7 items-center justify-center rounded-md transition",
        ...
        >
          {copied ? (
            <Check className="code-copy-confirmation h-4 w-4 animate-in fade-in zoom-in-75" />
          ) : (
              ...
              {tokens.map((line, lineIndex) => {
                ...
                return (
                  <span
                    ...
                    {...lineProps}
                    className={cn(
                      "response-code-line code-block-line",
                      lineProps.className,
                    )}
                  >
