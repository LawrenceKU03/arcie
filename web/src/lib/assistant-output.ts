...
function toArtifact(value: Record<string, unknown>): AssistantArtifact | undefined {
  ...
  if (urls.length > 0) {
    return {
      kind: "source",
      label: `Opened ${urls.length} source${urls.length === 1 ? "" : "s"}`,
...
function isInternalArtifactObject(value: Record<string, unknown>): boolean {
  return ["query", "results", "url", "urls", "content", "path", "pattern", "matches", "files"].some(
...
export function parseAssistantOutput(
  ...
  while (cursor < content.length) {
    if (content[cursor] !== "{") {
      ...
      if (nextObject === -1) {
        if (streaming && bridge.length <= 160 && isInternalArtifactBridge(bridge)) {
